...
<Image src="/blogs/first/the_legacy_paradigm.png" />
...
<Image src="/blogs/first/puda_architecture_brief.png" caption="PUDA Architecture" />
...
* **Modularity:** By partitioning the stack into Agency, Interface, Communication, and Edge layers, PUDA ensures that the "brain" of the system is never hard-coded to the "nerves" and "muscles." This separation provides the foundation for hardware and LLM-agnosticism, allowing users to swap components as technology evolves.
* **Scalibility:** PUDA is made to scale, supporting up to 1,000,000 devices and 300,000 commands per second. Integration of a new machine designed to be straightforward and rapid, provided a good programmatic interface for the device.
...
The foundation of PUDA is a high-performance messaging backbone powered by [NATS](https://nats.io/). In this layer, every entity — whether a hardware machine, or a CLI tool, or a logger — operates as a first-class participant within a unique subject namespace. This message-centric design provides three core advantages:
...
* **Language Agnosticism:** Any service that can speak NATS can join the ecosystem, this means 11 officially supported languages and 18 community contributed.

### Agentic Layer

Sitting at the top of the stack, the Agentic Layer serves as the system's intelligence center. It decides *what* moves and *when* — reasoning over incoming telemetry to determine the next course of action. It contains our basic PUDA Skills, which are generic skills on how to use the PUDA CLI.
...
The primary interface is the PUDA CLI, a powerful command-line tool through which agents and users alike runs protocols, inspect device state, and query execution history. The CLI will be covered in depth in a forthcoming post; for now it is enough to understand that it is the single entry point into the PUDA runtime environment.
...
<Image src="/blogs/first/puda-cli-1.svg" />
...
The recommended path is using our [python template](https://github.com/PUDAP/edge-python-template), which ships with all the NATS communication logic pre-wired. This lets you focus entirely on device-specific logic and connection handling, rather than the plumbing of the runtime.

### Agent Integration

PUDA is headless by design — there is no bundled UI. The intended way to drive PUDA is through an AI Agent. We recommend [Claude](https://claude.ai), [Hermes Agent](https://hermes-agent.nousresearch.com/), or [Cursor](https://cursor.com) for finer control. Simply install the PUDA Agent Skills from `pudap/skills` to unlock:
...
When a human-facing view is genuinely useful sometimes, the recommended path is for the agent itself to generate a GUI dashboard — for example, using [Streamlit](https://streamlit.io) via the PUDA CLI. Human UIs become derived artifacts of the agent-native platform, not a parallel stack to maintain.
...
* Broader integration across common lab and industrial equipment
